...
from .discovery_registry import target_priority_map
...
def operational_queue(database_path: Path, limit: int = 12) -> list[OperationalRole]:
    ...
    target_priorities = target_priority_map(PROJECT_ROOT / "data" / "target_companies.starter.json")
    def priority_for(row: dict):
        return role_priority(row, target_priorities.get(str(row.get("company", "")).casefold()))
    candidates.sort(key=lambda row: (priority_for(row).score, _WORKFLOW_WEIGHT[row["workflow_status"]]), reverse=True)
    queue = []
