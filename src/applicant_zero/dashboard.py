...
def build_discovery_page(database_path: Path) -> str:
    ...
    employer_rows = employer_coverage_rows(PROJECT_ROOT / "data" / "target_companies.starter.json", board_path)
    coverage_table = "".join(
        f"<tr><td>{html.escape(str(row['company']))}</td><td>{html.escape(str(row['sector']))}</td><td>P{row['priority']}</td><td><span class='{'auto' if row['ats'] else 'research'}'>{html.escape(str(row['route']))}{(' · ' + html.escape(str(row['ats']))) if row['ats'] else ''}</span></td></tr>"
