...
def _daily_execution_panel(database_path: Path) -> str:
    """Turn the operational queue and refresh health into a short daily plan."""
    roles = operational_queue(database_path, limit=3)
    healthy, refresh_message = discovery_log_status(database_path.parent.parent)
    role_rows = "".join(
        f"<li><strong>{html.escape(role.title)}</strong><span>{html.escape(role.company)} · priority {role.daily_priority} · {html.escape(role.application_effort)} effort</span><small>{html.escape(role.next_action)}</small></li>"
        for role in roles
    ) or "<li><strong>No role is queued yet.</strong><small>Refresh discovery or import a suitable listing.</small></li>"
    health_class = "is-healthy" if healthy else "needs-attention"
    return f"""<section class='execution-panel'><div><p class='eyebrow'>Today’s workflow</p><h2>One clear operating plan</h2><p>Work through the highest-value roles first, then clear any human handoffs.</p><p><a href='/operations'>Open application operations</a> <a href='/actions'>View manual actions</a></p></div><div><ol>{role_rows}</ol><p class='refresh-health {health_class}'>{html.escape(refresh_message)}</p></div></section>"""
...
def build_page(database_path: Path) -> str:
    ...
    execution_panel = _daily_execution_panel(database_path)
