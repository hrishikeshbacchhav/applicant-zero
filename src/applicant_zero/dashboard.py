...
from .reporting import discovery_inventory_csv, outcome_summary, progress_breakdown, tracker_csv
...
from .storage import (
    ...
    initialise_database,
    latest_refresh_run,
    latest_source_measurements,
    recent_query_measurements,
    query_performance_summary,
    discovery_inventory_summary,
    list_inventory_employers,
    list_board_checks,
...
def build_discovery_page(database_path: Path, board_message: str = "") -> str:
    ...
    if database_path.exists():
        with sqlite3.connect(database_path) as connection:
            ...
            source_measurements = latest_source_measurements(connection)
            query_measurements = recent_query_measurements(connection, limit=18)
            query_performance = query_performance_summary(connection, days=30, limit=16)
            inventory = discovery_inventory_summary(connection)
            observed_employers = list_inventory_employers(connection, limit=30)
    else:
        board_checks = []
        ...
        query_measurements = []
        query_performance = []
        inventory = {"collected": 0, "canonical": 0, "companies": 0}
        observed_employers = []
    ...
    # A 1,500-employer registry is useful for discovery but not as an
    # unbounded browser table. Keep P1/P2 and the first research slice visible;
    # the target total and sector count make the remaining coverage explicit.
    displayed_employer_rows = employer_rows[:180]
    remaining_employer_rows = max(0, len(employer_rows) - len(displayed_employer_rows))
    coverage_table = "".join(
        ...
        for row in displayed_employer_rows
    )
    history_table = "".join(
    ...
    source_table = "".join(
        f"<tr><td>{html.escape(str(row['source']))}</td><td>{row['collected_count']}</td><td>{row['relevant_count']}</td><td>{row['distinct_count']}</td><td>{row['repeated_count']}</td><td>{row['request_count']}</td><td>{row['failure_count']}</td></tr>"
        for row in source_measurements
    ) or "<tr><td colspan='7'>Run a daily discovery refresh to record source contribution.</td></tr>"
    query_table = "".join(
        f"<tr><td>{html.escape(str(row['query']))}</td><td>{html.escape(str(row['location']))}</td><td>{row['request_count']}</td><td>{row['returned_count']}</td><td>{row['relevant_count']}</td><td>{html.escape(str(row['error']) or 'Completed')}</td></tr>"
        for row in query_measurements
    ) or "<tr><td colspan='6'>Run a broad-feed refresh to measure individual search terms.</td></tr>"
    query_performance_table = "".join(
        f"<tr><td>{html.escape(str(row['query']))}</td><td>{html.escape(str(row['location']))}</td><td>{row['runs']}</td><td>{row['returned']}</td><td>{row['relevant']}</td><td>{row['relevance_rate']}%</td><td>{row['issues']}</td><td>{html.escape(str(row['last_checked']))}</td></tr>"
        for row in query_performance
    ) or "<tr><td colspan='8'>Run broad-feed refreshes to build local query-performance evidence.</td></tr>"
    query_performance_section = f"""<section class='section-card'><h2>30-day query performance</h2><p class='muted'>Use this evidence to improve the campaign vocabulary. A high return count is not automatically a good result; the relevant column uses Applicant Zero’s existing location, role and eligibility checks.</p><div class='table-wrap'><table><thead><tr><th>Search term</th><th>Location</th><th>Runs</th><th>Returned</th><th>Relevant</th><th>Relevant rate</th><th>Issues</th><th>Last checked</th></tr></thead><tbody>{query_performance_table}</tbody></table></div></section>"""

    observed_employer_table = "".join(
        f"<tr><td>{html.escape(str(row['company']))}</td><td>{row['distinct_listings']}</td><td>{row['source_records']}</td><td>{html.escape(str(row['sources'] or ''))}</td><td>{html.escape(str(row['last_seen_at']).split(' ')[0])}</td></tr>"
        for row in observed_employers
    ) or "<tr><td colspan='5'>Employers will appear here as discovery sources collect listings.</td></tr>"
    ...
    class DashboardHandler(BaseHTTPRequestHandler):
        def do_GET(self):
            ...
            if parsed.path == "/brief":
                ...
                self.wfile.write(content)
                return
            elif parsed.path == "/export-inventory":
                content = discovery_inventory_csv(database_path).encode("utf-8")
                self.send_response(200)
                self.send_header("Content-Type", "text/csv; charset=utf-8")
                self.send_header("Content-Disposition", "attachment; filename=applicant-zero-discovery-inventory.csv")
                self.send_header("Content-Length", str(len(content)))
                self.end_headers()
                self.wfile.write(content)
                return
